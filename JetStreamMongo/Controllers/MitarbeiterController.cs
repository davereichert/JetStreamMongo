...
public class MitarbeiterController : ControllerBase
{
    ...
    {
        var mitarbeiterDtos = await _mitarbeiterService.GetAllMitarbeiterAsync();
    ...
    {
        var mitarbeiterDtos = await _mitarbeiterService.CreateMitarbeiterAsync(createDto);
        return Ok(mitarbeiterDtos);
    }
    [HttpGet("{id}")]
    public async Task<ActionResult<GetMitarbeiterResponseDTO>> GetById(string id)
    {
        var mitarbeiter = await _mitarbeiterService.GetMitarbeiterByIdAsync(id);
        if (mitarbeiter == null)
        {
            return NotFound();
        }
    ...
    {
        await _mitarbeiterService.UpdateMitarbeiterAsync(id, mitarbeiterDto);
    ...
    {
        await _mitarbeiterService.DeleteMitarbeiterAsync(id);
